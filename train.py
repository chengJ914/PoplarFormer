...
try:
    ...
    try:
        from torch.cuda.amp import autocast, GradScaler as GradScaler_backup
        GradScaler = GradScaler_backup
    except ImportError:
        class GradScaler:
            def __init__(self):
                pass
...
def get_args_parser():
    ...
    parser.add_argument('--poplar_lr', default=2e-4, type=float,
...
# 使用统一的extract_numeric_value_for_coral函数
...
def main(args):
    ...
    if args.use_differential_lr:
        ...
        print(f"  - 新增模块学习率: {args.poplar_lr}")
    else:
    ...
    # 检查预训练权重文件 - 必须存在才能运行
    weights_path = os.path.abspath(local_weights) if not os.path.isabs(local_weights) else local_weights
    if not os.path.exists(weights_path):
        raise FileNotFoundError(
            f"错误: 预训练权重文件不存在: {local_weights}\n"
            "必须提供完整的预训练权重才能运行，不允许随机初始化。\n"
            "请确保权重文件路径正确，或修改 train.py 中的 local_weights 路径。"
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.use_differential_lr:
            ...
            poplar_lr = optimizer.param_groups[1]['lr'] if len(optimizer.param_groups) > 1 else 0
            history['lr'].append({'backbone_lr': backbone_lr, 'poplar_lr': poplar_lr})
        else:
            # 统一学习率模式
            current_lr = optimizer.param_groups[0]['lr']
            history['lr'].append({'backbone_lr': current_lr, 'poplar_lr': current_lr})
        ...
        print(f"学习率 - EVA02主干: {current_lr['backbone_lr']:.2e}, 新增模块: {current_lr['poplar_lr']:.2e}")
